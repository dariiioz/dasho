...
const ServiceTile = memo(function ServiceTile({
  ...
  return (
    <Card ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn("group relative min-w-0 gap-0 overflow-hidden rounded-xl border-border/90 bg-card py-0 shadow-none transition-[border-color,background-color] duration-150 ease-out hover:border-service-hover-border hover:bg-service-hover focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-ring/25", isDragging && "opacity-45", list && "rounded-lg")}>
      <a href={service.url} target={service.openInNewTab ? "_blank" : undefined} rel="noreferrer" aria-label={`Ouvrir ${service.name}`} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
...
function ServiceGrid({ services, editing, sortable, viewMode, cardSize, statuses, actions, columns }: { services: Service[]; editing: boolean; sortable: boolean; viewMode: "grid" | "list"; cardSize: SettingsDraft["cardSize"]; statuses: Map<number, ServiceStatus | undefined>; actions: ServiceActions; columns: number }) {
  ...
  return (
    <div className={cn("grid gap-2 [content-visibility:auto]", viewMode === "list" && "grid-cols-1")} style={viewMode === "grid" ? { gridTemplateColumns: `repeat(auto-fill, minmax(min(100%, max(${minimum}px, ${Math.floor(1280 / columns)}px)), 1fr))` } : undefined}>
      {services.map((service) => <ServiceTile key={service.id} service={service} editing={editing} sortable={sortable} viewMode={viewMode} cardSize={cardSize} status={statuses.get(service.id)} actions={actions} />)}
    </div>
  );
}

function FolderSection({ folder, services, editing, sortable, viewMode, cardSize, statuses, actions, columns, onEdit, onToggle }: { folder: Folder; services: Service[]; editing: boolean; sortable: boolean; viewMode: "grid" | "list"; cardSize: SettingsDraft["cardSize"]; statuses: Map<number, ServiceStatus | undefined>; actions: ServiceActions; columns: number; onEdit: (folder: Folder) => void; onToggle: (folder: Folder, open: boolean) => void }) {
  const { attributes, listeners, setActivatorNodeRef, setNodeRef, transform, transition, isDragging } = useSortable({ id: `folder-${folder.id}`, data: { type: "folder", folder }, disabled: !sortable });
  return (
    <section ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn("mb-6 scroll-mt-20 [content-visibility:auto]", isDragging && "opacity-45")} aria-labelledby={`folder-${folder.id}-title`}>
...
export function Dashboard() {
  ...
  const unfiledDrop = useDroppable({ id: "folder-null", data: { type: "folder-null" }, disabled: !editing });
  ...
  const deleteServiceMutation = useMutation({ mutationFn: deleteService, onSuccess: () => { toast.success("Service supprimé"); setServiceToDelete(null); void refresh(); }, onError: () => toast.error("Suppression impossible.") });
  ...
  const sortable = editing && parsedSettings.viewMode !== "tags" && parsedSettings.sortMode === "manual";
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    const targetFolderId = overService?.folderId ?? (over.data.current?.type === "folder" ? (over.data.current.folder as Folder).id : over.data.current?.type === "folder-null" ? null : activeService.folderId);
  ...
  const renderGroupedSections = () => data.folders.map((folder) => {
    ...
    return <FolderSection key={folder.id} folder={folder} services={services} editing={editing} sortable={sortable} viewMode={parsedSettings.viewMode === "list" ? "list" : "grid"} cardSize={parsedSettings.cardSize} statuses={statuses} actions={actions} columns={parsedSettings.columns} onEdit={(item) => setFolderDraft({ ...item })} onToggle={toggleFolder} />;
  });
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
      <div data-accent={parsedSettings.accent} className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,var(--color-primary)/0.04,transparent_28rem)]">
        ...
        <AlertDialog open={serviceToDelete !== null} onOpenChange={(open) => { if (!open && !deleteServiceMutation.isPending) setServiceToDelete(null); }}><AlertDialogContent><AlertDialogHeader><AlertDialogMedia className="bg-destructive/10 text-destructive"><Trash2 aria-hidden="true" /></AlertDialogMedia><AlertDialogTitle>Supprimer « {serviceToDelete?.name} » ?</AlertDialogTitle><AlertDialogDescription>Ce lien sera définitivement supprimé du dashboard. Cette action est irréversible.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={deleteServiceMutation.isPending}>Annuler</AlertDialogCancel><AlertDialogAction variant="destructive" disabled={deleteServiceMutation.isPending} onClick={() => serviceToDelete && deleteServiceMutation.mutate(serviceToDelete.id)}><Trash2 data-icon="inline-start" />{deleteServiceMutation.isPending ? "Suppression…" : "Supprimer le lien"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
